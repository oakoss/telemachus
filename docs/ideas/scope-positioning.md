...
- **Own the seams + the wedge; rent solved-problem implementations.** "Build-our-own" scopes to the **wedge / agent-core** (agent loop, run/step model, memory, glass-box, reactive sync) — _not_ generic infrastructure. We freely depend on maintained, low-dep, OSS libraries for solved problems (validation, IDs, logging, ORM, dates) when they meet our hard requirements (self-host/license **and** seams like the injected clock for determinism), **wrapped behind a thin interface we own so they stay swappable**. Own it instead when it _is_ the wedge, when a lib fails a hard constraint, or when it's tiny enough that owning beats the wrapper. Smell test: _"if this lib went unmaintained, how trapped are we?"_ — low (behind a seam) → rent; high (woven through the wedge) → own.
...
The stack is no longer a neutral recommendation — **"better stack" is part of the _why_**: **TypeScript is the lane** (vs Odysseus's Python), on a TanStack-leaning stack the author owns and extends. The stack is **fully decided** — the ADRs ([`../decisions/`](../decisions/)) are the source of truth (stack-at-a-glance in [`../decisions/README.md`](../decisions/README.md)), built on the 4-candidate evaluation in [`../research/stack-data-layer.md`](../research/stack-data-layer.md). Remaining open items are **per-thread design** (e.g. where local-first sync enters, thread #5), not stack.
...
- **Stack:** fully decided — the ADRs ([`../decisions/`](../decisions/)) are the source of truth (stack-at-a-glance in [`../decisions/README.md`](../decisions/README.md)). Open items are **per-thread design** (e.g. sync entry, thread #5), not stack.
