...
The stack is no longer a neutral recommendation — **"better stack" is part of the _why_.** **TypeScript is the lane** (vs Odysseus's Python). The **data layer is decided:** TanStack DB + ElectricSQL, with **SQLite on-device** ([ADR-001](../decisions/001-data-layer-tanstack-db-electric-pglite.md) + its amendment), after the 4-candidate evaluation in [`../research/stack-data-layer.md`](../research/stack-data-layer.md). The **app framework + deploy** ([ADR-002](../decisions/002-app-framework-tanstack-start-nitro.md): TanStack Start + Nitro), the **ORM** ([ADR-003](../decisions/003-orm-drizzle.md): Drizzle), **auth** ([ADR-004](../decisions/004-auth-better-auth.md): Better Auth), and the **UI layer** ([ADR-005](../decisions/005-ui-react-aria-intent.md): react-aria-components + Intent UI, copy-paste) are now decided too. The **model/LLM layer** ([ADR-006](../decisions/006-model-llm-layer.md): TanStack AI + pluggable Ollama serving, build-our-own agent loop) and the **repo shape + toolchain** ([ADR-007](../decisions/007-repo-shape-and-toolchain.md): Turborepo · Vitest/Playwright · Zod v4 · XState Store) are decided too. The **server runtime** (Node now, revisit Bun) and the **architecture/topology** ([ADR-008](../decisions/008-architecture-and-topology.md): modular `packages/*` · Hono `core` · Start BFF · PWA→Tauri) complete it — **the stack is fully decided** ([ADR-001](../decisions/001-data-layer-tanstack-db-electric-pglite.md)–[008](../decisions/008-architecture-and-topology.md)). Remaining open items are per-thread design (e.g. where local-first sync enters, thread #5), not stack. See [`thesis.md`](thesis.md).
