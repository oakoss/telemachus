...
- **(now) Developer environment + local services** — one documented toolchain (mise: Node/pnpm) + `docker-compose` for dev backing services (Postgres now; Electric/MinIO later) + `.env.example`. The app runs on the host for dev; Docker packages the deploy image (Coolify) + runs services + test databases (Testcontainers, real Postgres). Ollama is native on macOS (Metal GPU), Docker on Linux. The wa-sqlite OPFS VFS needs **no** COOP/COEP (telemachus-8zj.3) — the sync VFS uses no SharedArrayBuffer, and the `/db-smoke` e2e asserts `crossOriginIsolated === false`. _Recorded: R1-scaffold (E0)._
