...
| Job             | Does                                                                                                                                                   | Notes                                                                                                                                                                                                       |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **static**      | `oxfmt --check` + `oxlint` (root; type-aware via `.oxlintrc` config) · `turbo lint` (ESLint, per-package) · `markdownlint` · `knip` + `sherif` · `tsc` | oxlint runs type-aware (tsgolint) in CI; ESLint carries only the remaining gap, deduped against the oxlint config ([ADR-007](../decisions/007-repo-shape-and-toolchain.md)); knip/sherif = monorepo hygiene |
| **unit**        | Vitest unit                                                                                                                                            | no DB                                                                                                                                                                                                       |
| **integration** | Vitest + **Testcontainers** (real Postgres)                                                                                                            | `ubuntu-latest` (Docker preinstalled); Linux only                                                                                                                                                           |
| **e2e**         | Playwright (+ axe a11y)                                                                                                                                | smoke + a11y always; Electric write-path specs run against a Testcontainers Postgres+Electric stack (`test:e2e:containers`, `REQUIRE_ELECTRIC=1`)                                                           |
