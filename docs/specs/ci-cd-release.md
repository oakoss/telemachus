...
| Job             | Does                                                                                                                             | Notes                                                                                                                                                                                                       |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **static**      | `oxfmt --check` + `oxlint --type-aware` (root) · `turbo lint` (ESLint, per-package) · `markdownlint` · `knip` + `sherif` · `tsc` | oxlint runs type-aware (tsgolint) in CI; ESLint carries only the remaining gap, deduped against the oxlint config ([ADR-007](../decisions/007-repo-shape-and-toolchain.md)); knip/sherif = monorepo hygiene |
