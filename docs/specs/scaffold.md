...
- `packages/shared` — the foundations utilities R1 needs: UUIDv7 ids, Zod-validated env, a logger behind an interface (console default at `.4`, Pino server-side later; + a correlation-id field), and a typed result/error shape.
- `packages/core` — a Hono app skeleton mounted **in-process** behind the Start BFF (a `/health` + `/version` route); not a separate service.
- A strict **CSP** on the app from the first commit — only `worker-src 'self'` is needed (telemachus-8zj.3): the wasm is base64-inlined and compiled inside the same-origin worker, so no `wasm-unsafe-eval`, no `blob:`, no separate asset origin (rationale in `apps/web/src/lib/csp.ts`). Pairs with the reproducible-WASM-bootstrap + SRI seam ([`foundations.md`](foundations.md)).
- **Dev environment + services** — a `docker-compose.yml` for **dev backing services** (Postgres; Electric behind the `electric` profile, added in the telemachus-8zj.8 spike; MinIO at a later rung), a production **`Dockerfile`** (Nitro build → Coolify), `.env.example`, and a dev-setup doc ([`../development/dev-environment.md`](../development/dev-environment.md)). The **app runs on the host for dev** (`pnpm dev`); Docker is for **dev services + test databases (Testcontainers) + the deploy image**, not the dev app.
...
| `packages/eslint`     | ESLint flat-config presets (node/browser/test/typeChecked + `config()`) + plugin deps                              | imported per-package; **oxlint/oxfmt configs live at the repo root** (oxlint via nested per-package configs)         |
...
4. **Foundations utils** — `packages/shared`: UUIDv7, Zod env, logger interface + console default (+ correlation id, redaction; Pino later), result/error.
