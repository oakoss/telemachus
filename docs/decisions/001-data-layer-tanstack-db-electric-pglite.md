...
- **Client (browser/native): TanStack DB collections + first-party SQLite persistence** (`@tanstack/*-sqlite-persistence`) — OPFS built in; multi-tab leader election exists in the core protocol but the browser adapter ships single-tab today (see _Re-verified 2026-06-07_).
